...
here, fails too. Here that list names `CONFORMANCE.md` and the four JSON Schemas. And the version
itself is checked: a tree faithful to the tag it claims while canonical has tagged a newer one is
a warning in CI and, with `--require-current`, a refusal in the release job, because a package
must not be published claiming a spec version canonical has moved past.
